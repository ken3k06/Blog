...
5. $\mathcal{C}$ chọn ngẫu nhiên $\displaystyle b\leftarrow \{0,1\}$ và mã hóa $c^{*} \leftarrow Enc(k,m_b^{*})$ rồi gửi lại cho $\mathcal{A}$. 
...
1. $\mathcal{C}$ sinh khóa $k \leftarrow Gen(1^\lambda)$  , $k = (pk,sk)$ 
